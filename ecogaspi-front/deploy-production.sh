...
ssh -i ~/.ssh/id_rsa_ecogaspi $SERVER_USER@$SERVER_HOST "cat > /tmp/deploy-ecogaspi-front.sh" << 'DEPLOY_SCRIPT'
#!/bin/bash
set -e
...
# Libérer le port 80 si utilisé par l'API (pour éviter les conflits)
echo "🔧 Vérification des ports..."
if docker ps --format "table {{.Names}}\t{{.Ports}}" | grep ":80->" | grep -v ecogaspi-front; then
    echo "⚠️ Port 80 utilisé par un autre service. Vérifiez votre configuration."
fi
...
for i in {1..20}; do
    if curl -f http://localhost:8082/health 2>/dev/null; then
        ...
        echo "   - Frontend: http://$(hostname):8082"
        echo "   - Health: http://$(hostname):8082/health"
        break
        ...
        docker-compose -f docker-compose.prod.yml logs ecogaspi-front --tail=50
        exit 1
    else
...
ssh -i ~/.ssh/id_rsa_ecogaspi $SERVER_USER@$SERVER_HOST "
    chmod +x /tmp/deploy-ecogaspi-front.sh
    /tmp/deploy-ecogaspi-front.sh $DOCKER_IMAGE
    rm -f /tmp/deploy-ecogaspi-front.sh
"

echo ""
echo "🎉 Déploiement terminé avec succès!"
echo "🌐 Votre application est maintenant disponible sur:"
echo "   - http://$SERVER_HOST:8082"
echo "   - http://$SERVER_HOST:8082/health"
echo ""
...
echo "   - Restart: ssh -i ~/.ssh/id_rsa_ecogaspi $SERVER_USER@$SERVER_HOST 'cd $SERVER_PATH && docker-compose -f docker-compose.prod.yml restart ecogaspi-front'"